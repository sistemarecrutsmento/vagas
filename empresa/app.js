...
async function carregarEquipe(){const box=document.getElementById('equipe-content');if(box)box.innerHTML='<div class="equipe-loading"><span class="spinner"></span> Carregando equipe...</div>';try{const r=await fetch(API+'/api/empresa/usuarios',{headers:{'Authorization':'Bearer '+token}}),d=await r.json();if(!r.ok)throw new Error(d.erro||'Não foi possível carregar a equipe');equipeRowsReal=d.usuarios||[];const active=equipeRowsReal.filter(u=>u.ativo!==false);const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};set('equipe-kpi-ativos',active.length);set('equipe-kpi-admins',equipeRowsReal.filter(u=>u.role==='admin_empresa').length);set('equipe-kpi-recrutadores',equipeRowsReal.filter(u=>u.role==='recrutador').length);const invite=document.getElementById('equipe-kpi-convites')?.closest('.equipe-summary-card');if(invite)invite.style.display='none';renderEquipe();}catch(e){if(box)box.innerHTML=`<div class="equipe-empty"><strong>Não foi possível carregar os dados da equipe.</strong><p>${escapeHtml(e.message||'Tente novamente.')}</p><button class="btn btn-sec" type="button" onclick="carregarEquipe()">Tentar novamente</button></div>`;}}
...
function abrirModalNovoMembroEquipe(){['equipe-novo-nome','equipe-novo-email','equipe-novo-cargo','equipe-novo-senha'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='';});const a=document.getElementById('equipe-modal-alert');if(a)a.innerHTML='';abrirModal('equipe-novo');}
async function salvarNovoMembroEquipe(){const nome=document.getElementById('equipe-novo-nome').value.trim(),email=document.getElementById('equipe-novo-email').value.trim(),cargo=document.getElementById('equipe-novo-cargo').value.trim(),role=document.getElementById('equipe-novo-role').value,senha=document.getElementById('equipe-novo-senha').value,a=document.getElementById('equipe-modal-alert');if(!nome||!email||!senha||senha.length<6){a.innerHTML='<div class="alert alert-erro">Preencha nome, e-mail e uma senha provisória com pelo menos 6 caracteres.</div>';return;}try{const r=await fetch(API+'/api/empresa/usuarios',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({nome,email,cargo,role,senha})}),d=await r.json();if(!r.ok)throw new Error(d.erro||'Não foi possível criar o acesso');fecharModal('equipe-novo');alert('Usuário criado com sucesso.');await carregarEquipe();}catch(e){a.innerHTML=`<div class="alert alert-erro">${escapeHtml(e.message)}</div>`;}}
...
const configState={tab:'empresa',empresa:null,totp:null};
async function carregarConfiguracoes(){const box=document.getElementById('config-content');if(box)box.innerHTML='<div class="config-loading"><span class="spinner"></span> Carregando configurações...</div>';try{const h={'Authorization':'Bearer '+token};const [er,tr]=await Promise.all([fetch(API+'/api/empresa/minha-empresa',{headers:h}),fetch(API+'/api/empresa/2fa/status',{headers:h})]);const ed=await er.json(),td=await tr.json();if(!er.ok)throw new Error(ed.erro||'Não foi possível carregar a empresa');configState.empresa=ed.empresa;configState.totp=tr.ok?td:null;renderConfiguracoes();}catch(e){if(box)box.innerHTML=`<div class="config-empty"><strong>Não foi possível carregar esta configuração.</strong><p>${escapeHtml(e.message||'Tente novamente.')}</p><button class="config-btn" type="button" onclick="carregarConfiguracoes()">Tentar novamente</button></div>`;}}
...
function propostaStatus(c){if(c.proposta_aceita_em)return'aceita';if(c.proposta_recusada_em||['rejeitado','reprovado'].includes(c.status))return'recusada';if(c.proposta_enviada_em)return'enviada';if(Number(c.etapa_atual)===5)return'rascunho';return null;}

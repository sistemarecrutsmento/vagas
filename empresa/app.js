...
// ===== RELATÓRIOS =====
const relatoriosState={periodo:'30',inicio:null,fim:null,vaga:'',etapa:'',area:'',localidade:'',responsavel:'',status:'',filial:'',chart:'candidatos'};let relatoriosData={candidaturas:[],entrevistas:[],vagas:[]};
...
function relatorioTodosExportRows(){const tipos=['resumo-vagas','resumo-candidatos','resumo-entrevistas','resumo-propostas','resumo-contratacoes','resumo-tempo','diversidade','evolucao','funil','entrevistas-resumo','propostas-resumo','vagas','tempo'],labels={'resumo-vagas':'Resumo — Vagas','resumo-candidatos':'Resumo — Candidatos','resumo-entrevistas':'Resumo — Entrevistas','resumo-propostas':'Resumo — Propostas','resumo-contratacoes':'Resumo — Contratações','resumo-tempo':'Resumo — Tempo médio',diversidade:'Diversidade e localização',evolucao:'Evolução',funil:'Funil','entrevistas-resumo':'Desempenho das entrevistas','propostas-resumo':'Desempenho das propostas',vagas:'Desempenho das vagas',tempo:'Tempo do processo'},out=[['Relatório','Campo/Indicador','Valor ou detalhe 1','Valor ou detalhe 2','Valor ou detalhe 3','Valor ou detalhe 4','Valor ou detalhe 5']];tipos.forEach(tipo=>{const rows=relatorioExportRows(tipo);rows.slice(1).forEach(row=>{const line=[labels[tipo],...row];while(line.length<7)line.push('');out.push(line);});});return out;}
function baixarTodosRelatorios(){baixarCsv('relatorios-vagasio-completo',relatorioTodosExportRows());}
function baixarTodosPdf(){baixarPdf('relatorios-vagasio-completo',relatorioPdfLinesFromRows('Relatórios completos',relatorioTodosExportRows()));}

...
async function carregarDashboardBase() {
  try {
    ...
    const labels = [...new Set((configuredLabels.length ? configuredLabels : (data.etapas_labels || ['Inscrição', 'Triagem', 'RH', 'Gestor', 'Proposta', 'Coleta Docs', 'Contratação'])).map(x => String(x).trim()).filter(Boolean))].slice(0, 7);

...
function atualizarResumoRelatorios(cands,ints,vagas){const proposals=cands.filter(c=>c.proposta_enviada_em&&relatorioInPeriod(c.proposta_enviada_em)),hires=cands.filter(c=>c.status==='contratado'&&relatorioInPeriod(c.atualizada_em)),active=cands.filter(c=>!['contratado','reprovado','rejeitado','cancelado'].includes(c.status));const open=vagas.filter(v=>v.status==='publicada').length,avg=relatorioAvgDays(hires,'proposta_aceita_em','atualizada_em');const vals={vagas:open,candidatos:relatorioUnique(active,'candidato_id'),entrevistas:ints.length,propostas:proposals.length,contratacoes:hires.length,tempo:avg||'—',conversao:relatorioPercent(hires.length,relatorioUnique(cands,'candidato_id'))};document.querySelectorAll('[data-report]').forEach(e=>e.textContent=vals[e.dataset.report]??'—');}

...
function abrirModalNovaProposta(){const s=document.getElementById('proposta-candidatura');if(!s)return;const eligible=propostasAll.length?propostasAll.filter(c=>!c.proposta_enviada_em&&Number(c.etapa_atual)===propostaEtapaIndex(c)):[];s.innerHTML='<option value="">Selecione uma candidatura na etapa de proposta...</option>'+eligible.map(c=>`<option value="${c.id}">${escapeHtml(c.candidato_nome||'Candidato')} · ${escapeHtml(c.titulo||'Vaga')}</option>`).join('');document.getElementById('proposta-texto').value='';document.getElementById('proposta-pdf-file').value='';document.getElementById('alert-proposta').innerHTML=eligible.length?'':'<div class="alert alert-erro">Não há candidaturas elegíveis para uma nova proposta.</div>';abrirModal('proposta');}
async function enviarNovaProposta(){const id=document.getElementById('proposta-candidatura').value,texto=document.getElementById('proposta-texto').value.trim(),file=document.getElementById('proposta-pdf-file').files[0];if(!id||(!texto&&!file)){document.getElementById('alert-proposta').innerHTML='<div class="alert alert-erro">Selecione uma candidatura e informe o texto ou anexe um PDF.</div>';return;}if(file&&(file.type!=='application/pdf'||file.size>6*1024*1024)){document.getElementById('alert-proposta').innerHTML='<div class="alert alert-erro">Anexe somente um PDF de até 6 MB.</div>';return;}try{let pdf_url=null;if(file)pdf_url=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file);});const r=await fetch(API+'/api/empresa/candidatura/'+id+'/proposta',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({texto,pdf_url,pdf_nome:file?file.name:null})});const d=await r.json();if(!r.ok)throw new Error(d.erro||'Não foi possível enviar a proposta');fecharModal('proposta');await carregarPropostas();}catch(e){document.getElementById('alert-proposta').innerHTML=`<div class="alert alert-erro">${escapeHtml(e.message)}</div>`;}}

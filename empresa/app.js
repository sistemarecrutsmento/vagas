...
async function carregarVideoConfigEmpresa(){
  try {
    const r = await fetch(API + '/api/video/config', {headers:{Authorization:'Bearer '+token}, cache:'no-store'});
    if (!r.ok) return;
    const c = await r.json();
    empresaVideoFeature = c.enabled === true && c.role === 'recruiter' && /^wss:\/\//i.test(c.signalUrl || '');
  } catch (_) { empresaVideoFeature = false; }
}
function entrarVideoEmpresa(id){
  if (!empresaVideoFeature) return false;
  sessionStorage.setItem('vagasio_video_interview_id', String(id));
  location.href = 'video-call.html';
  return true;
}
...
// IA projection for the global candidate list. Results are tenant-scoped by the API
// and cached for this page session to avoid repeating analysis requests.
const candidatosAnaliseIaCache = new Map();
...
async function carregarCandidatos() {
  const table=document.querySelector('#candidatos-table tbody'); const mobile=document.getElementById('candidatos-mobile-list'); if(table)table.innerHTML='<tr><td colspan="8" class="empty"><div class="spinner"></div></td></tr>'; if(mobile)mobile.innerHTML='';
  popularSelectAreas();
  try {
    ...
    hidratarAnalisesCandidatos(rows);
  } catch(e) { if(table)table.innerHTML=`<tr><td colspan="8" class="empty">${escapeHtml(e.message||'Erro ao carregar candidatos')}</td></tr>`; }
}
function renderCandidatos() {
  const table=document.querySelector('#candidatos-table tbody'), mobile=document.getElementById('candidatos-mobile-list'); const rows=candidatosRows; if(!rows.length){if(table)table.innerHTML='<tr><td colspan="8" class="empty">Nenhum candidato corresponde aos filtros selecionados.</td></tr>';if(mobile)mobile.innerHTML='<div class="candidatos-empty-mobile">Nenhum candidato corresponde aos filtros selecionados.</div>';}

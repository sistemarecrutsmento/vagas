...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (typeof window.authInit === 'function') {
    ...
    if (refreshed) {
      ...
      token = localStorage.getItem('empresa_token');
...
async function fazerLogin() {
  ...
  try {
    ...
    // 1ª tentativa: admin
    let r = await authedFetch(API + '/api/auth/login-empresa', {
    ...
    if (r.ok && data.token) {
      ...
      if (data.refreshToken) {
        localStorage.setItem('empresa_refresh', data.refreshToken);
      }
...
async function verificar2FA() {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/__noop', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function salvarNovoRecrutador() {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '#saas-only/recrutadores', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function salvarNovaEmpresa() {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '#saas-only/empresas', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function salvarEdicaoRecrutador() {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '#saas-only/recrutadores/' + id, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
...
function excluirRecrutador(id, nome) {
  ...
  authedFetch(API + '#saas-only/recrutadores/' + id, {
    method: 'DELETE',
    headers: {}
  })
    .then(r => r.json())
...
async function salvarEdicaoEmpresa() {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '#saas-only/empresas/' + id, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
function excluirEmpresa(id, nome) {
  ...
  authedFetch(API + '#saas-only/empresas/' + id, {
    method: 'DELETE',
    headers: {}
  })
    .then(r => r.json())
...
async function carregarVincularVagas() {
  ...
  try {
    ...
    const [rVagas, rLiberadas] = await Promise.all([
      authedFetch(API + '/api/empresa/vagas', { headers: {} }),
      authedFetch(API + '#saas-only/empresa-vaga/' + vincEmpresaId, { headers: {} })
    ]);
...
async function vincularVagaEmpresa(vagaId) {
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '#saas-only/empresa-vaga', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function salvarUsuarioEmpresa() {
  ...
  try {
    ...
    if (id) {
      ...
      r = await authedFetch(API + '#saas-only/empresa-usuarios/' + id, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Criar
      r = await authedFetch(API + '#saas-only/empresas/' + empresaId + '/usuarios', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function excluirUsuarioEmpresa(id, nome) {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '#saas-only/empresa-usuarios/' + id, {
      method: 'DELETE',
      headers: {}
    });
    const d = await r.json();
...
async function carregarAgenda(periodo) {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/entrevistas?periodo=' + agendaPeriodoAtual, {
      headers: {}
    });
    const data = await r.json();
...
async function atualizarEntrevista(id, status) {
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '/api/empresa/entrevista/' + id, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
async function cancelarEntrevistaFalhou(id) {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  try {
    const r = await authedFetch(API + '/api/empresa/entrevista/' + id + '/cancelar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function abrirModalNovaEntrevista() {
  ...
  const token = localStorage.getItem('empresa_token') || localStorage.getItem('token');
  authedFetch(API + '/api/empresa/entrevista', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
...
async function abrirModalContratacoes(event) {
  ...
  const _token = localStorage.getItem('empresa_token') || sessionStorage.getItem('empresa_token') || sessionStorage.getItem('recrutador_token') || '';
  try {
    const r = await authedFetch(API + '/api/empresa/contratacoes', {
      headers: {}
    });
    const data = await r.json();
...
async function abrirModalVagasAntigas(event) {
  ...
  const _token = localStorage.getItem('empresa_token') || sessionStorage.getItem('empresa_token') || sessionStorage.getItem('recrutador_token') || '';
  try {
    const r = await authedFetch(API + '/api/empresa/vagas-abertas-antigas', {
      headers: {}
    });
    const data = await r.json();
...
async function abrirModalCandidatosEtapa(etapaNum, etapaNome) {
  ...
  try {
    const _token = localStorage.getItem('empresa_token') || sessionStorage.getItem('empresa_token') || sessionStorage.getItem('recrutador_token') || sessionStorage.getItem('token');
    const r = await authedFetch(API + '/api/empresa/candidaturas-por-etapa?etapa=' + etapaNum, {
      headers: {}
    });
    const data = await r.json();
    if (!r.ok) {
...
async function abrirModalVagasAtivas(event) {
  ...
  const _token = localStorage.getItem('empresa_token') || sessionStorage.getItem('empresa_token') || sessionStorage.getItem('recrutador_token') || '';
  try {
    const r = await authedFetch(API + '/api/empresa/vagas?status=publicada', {
      headers: {}
    });
    const data = await r.json();
    const vagas = data.vagas || [];
...
async function carregarDashboardV2() {
  try {
    const r = await authedFetch(API + '/api/empresa/dashboard', { headers: {} });
    const data = await r.json();
    if (!r.ok) {
...
async function carregarVagasAdminNovo() {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/vagas?' + params.toString(), {
      headers: {}
    });
    const data = await r.json();
...
function atualizarAbasVagas() {
  ...
  ['todas', 'publicadas', 'fechadas', 'pausadas'].forEach(async (key) => {
    ...
    try {
      const r = await authedFetch(API + '/api/empresa/vagas?status=' + statusMap[key] + '&limit=1', {
        headers: {}
      });
      const d = await r.json();
...
async function duplicarVagaAdmin(id) {
  ...
  try {
    // Busca dados da vaga
    const r1 = await authedFetch(API + '/api/empresa/vagas/' + id, { headers: {} });
    ...
    const r2 = await authedFetch(API + '/api/empresa/vagas', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function atualizarStatusVagaAdmin(id, novoStatus, msg) {
  try {
    const r = await authedFetch(API + '/api/empresa/vagas/' + id, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
async function editarVaga(id) {
  try {
    const r = await authedFetch(API + '/api/empresa/vagas/' + id, { headers: {} });
...
async function abrirCurriculo(id) {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/candidatos/' + id, { headers: {} });
    if (!r.ok) {
...
async function carregarCandidaturas() {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/vagas-com-candidaturas', { headers: {} });
    if (!r.ok) {
      const err = await r.json().catch(() => ({}));
...
async function abrirVagasFechadasSemContratacao() {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/vagas-fechadas-sem-contratacao', { headers: {} });
...
async function acaoCandidatura(id, acao) {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/candidatura/' + id + '/status', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function verCandidatura(id) {
  ...
  try {
    const r = await authedFetch(API + '/api/empresa/candidatura/' + id, { headers: {} });
    const data = await r.json();
    if (!r.ok) {
...
async function importarVagasDemo() {
  ...
  try {
    const r = await authedFetch(API + '#saas-only/seed', {
      method: 'POST',
      headers: {}
    });
  ...
  const html = `
    ...
    <body><h1>📜 Histórico</h1>
      ...
      <script>
        ...
        const API = 'https://recrutamento-api-novo.onrender.com';
        const t = localStorage.getItem('empresa_token');

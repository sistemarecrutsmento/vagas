...
const configState={tab:'empresa',empresa:null,totp:null,prefs:null,assinatura:null};
async function carregarConfiguracoes(){const box=document.getElementById('config-content');if(box)box.innerHTML='<div class="config-loading"><span class="spinner"></span> Carregando configurações...</div>';try{const h={'Authorization':'Bearer '+token};const [er,tr,pr,sr]=await Promise.all([fetch(API+'/api/empresa/minha-empresa',{headers:h}),fetch(API+'/api/empresa/2fa/status',{headers:h}),fetch(API+'/api/email/preferencias',{headers:h}),fetch(API+'/api/empresa/assinatura',{headers:h})]);const ed=await er.json(),td=await tr.json(),pd=await pr.json(),sd=await sr.json();if(!er.ok)throw new Error(ed.erro||'Não foi possível carregar a empresa');configState.empresa=ed.empresa;configState.totp=tr.ok?td:null;configState.prefs=pr.ok?(pd.preferencias||[]):null;configState.assinatura=sr.ok?(sd.assinatura||null):null;renderConfiguracoes();}catch(e){if(box)box.innerHTML=`<div class="config-empty"><strong>Não foi possível carregar esta configuração.</strong><p>${escapeHtml(e.message||'Tente novamente.')}</p><button class="config-btn" type="button" onclick="carregarConfiguracoes()">Tentar novamente</button></div>`;}}
